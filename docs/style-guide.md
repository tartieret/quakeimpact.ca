...
**A narrative run may defer its sourcing to the pages it links to.** Markers in running prose are how this site proves a claim, and in a four-panel story they turn the same five documents over nine times and make a sequence read like a filing. Nobody reads a filing who is not paid to. So a narrative passage may carry its attribution through its links instead, under conditions that hold every time: every sentence is a consequence a page on this site states and sources; the noun it hangs on is a link to that page; a quotation keeps its attribution, named in words where the speaker is the point and otherwise carried on a marker beside it, since a quote with nothing attached is worse than a claim without a marker; and the passage says in plain terms what it is, so nobody reads a scenario as a forecast. The home page timeline is the worked example. Everywhere else, the marker stays.

**In such a passage a duration may ride on its link. A count, a proportion or a measurement may not.** Revised 14 September 2026. The rule had been that no duration appears in prose carrying no marker, and it backfired: rather than break the sequence with a marker, the timeline started reporting on the state of the record instead — which utility had published an estimate and which had not — and a reader learned about this site's sourcing rather than about the weeks they would spend without power. How long something stays broken is the one thing this site exists to say. A sequence that cannot reach for *several weeks* or *months rather than weeks* without stopping to cite itself cannot do its job. So write the duration in the words the source uses, no tighter, name the speaker where the speaker is the point, and link the noun to the page that states it with every qualification attached. This is the habit the band cells already have: a band is a duration, and it travels with a source line rather than a numbered marker. What still takes its marker where it appears is everything countable: 267 water mains, two thirds of downtown customers, 95,000 customers, four litres per person per day. Those are the figures a reader repeats to somebody else, and a figure that travels without its document is how this site loses an argument it should win.

**Storytelling is not decoration on the evidence. It is what makes the evidence mean anything.** A reader told that wastewater disruption is expected for many months has a fact they will forget by the end of the paragraph. A reader taken through the week where the toilet stops working on the first day, the waste goes into buckets, and the tower has nowhere to put it understands why four litres a person a day is the number on the label and not the number they need. Severity without a story is a table, and severity without a lever is dread; the sequence is what carries a reader from one to the action at the end. None of this licenses invention: the scenes are the ones the record establishes, and the rules above about scenes, dashes and stacked fragments hold inside the narrative as tightly as outside it. Write well, keep it true, and let the sequence make the argument for preparing.
...
**A narrative passage describes the experience, not the state of the record, and not the document either.** The closest relative of the rule above, and the one that gets into a timeline. "BC Hydro has published an estimate for downtown Vancouver and for nowhere else in the region" and "no restoration estimate for the water network has been published at all" are both true, and both teach a reader about this site's evidence base rather than about the weeks they would spend without power or a working sewer. In a sequence, give the published duration where there is one, with its marker, and where there is none write what the mechanism does to an ordinary week: the sewers stay broken for months, waste goes in buckets, a tower has nowhere to put it. Naming the gap is a requirement on the page carrying the evidence, where it can be stated in full, and a distraction in the middle of the story.

The same goes for naming the document. "In the province's scenario the earthquake is heard before it is felt" and "in the province's own plan, family and neighbours are likely to be the only available first responders" both put a filing cabinet between the reader and the thing that happens, and repeated down four panels they turn a sequence into a reading of the bibliography. Write what happens; let the link, or a marker where one is needed, say who established it. Keep the body's name only where §6 already requires it, where removing the name removes the point: an owner conceding something against its own interest, or a plan whose assumption is itself the subject, as in the province planning on the basis that people stay in the region. That one cannot lose its speaker, because stated flat it becomes a claim about what people do rather than about what the plan assumes.
...
**Attribute everything.** Every impact band on this site carries a mechanism sentence and a source link — the component makes it structurally impossible to ship a coloured cell alone. Prose should meet the same bar. A sentence with a number in it and no source attached is a bug. Attached has three forms and the choice between them is a matter of what reads well: a numbered marker in ordinary prose, a source line under a band, a link to the page holding the document in a narrative run. What is never optional is that a reader can get from the claim to the document.
