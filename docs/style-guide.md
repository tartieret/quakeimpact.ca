...
**State the counterpoint before an expert does.** Where a comparison is provocative — the Berlin airlift capacity arithmetic, for instance — name its limits in the same section. An argument that anticipates its own objection is far stronger than one that has to be defended later.
